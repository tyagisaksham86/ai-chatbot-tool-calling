...
from app.db.mongo import sales_collection, admin_collection, chat_history
...
def chat_with_agent(user_message: str, session_id: str | None):
    ...
    if model_message.tool_calls:
        ...
        # -----------------------------------
        # SALES DATA (customer_id / customer_name)
        # -----------------------------------
        if source == "sales":
            if record_id is not None:
                record = sales_collection.find_one(
        ...
        # -----------------------------------
        # ADMIN DATA (admin_id / username)
        # -----------------------------------
        elif source == "admin":
            if record_id is not None:
                record = admin_collection.find_one(
                    {"admin_id": int(record_id)},
                    {"_id": 0}
                )
            elif username:
                record = admin_collection.find_one(
